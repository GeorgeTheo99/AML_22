...
class EnhancedEnsembleModel:
    ...
    def predict_weighted_voting(self, mlp_results, logreg_results, cnn_results):
        weighted_probs = (
            self.model_weights['mlp'] * mlp_results[1] +
            self.model_weights['logreg'] * logreg_results[1] +
            self.model_weights['cnn'] * cnn_results[1]
        )
        weighted_preds = np.argmax(weighted_probs, axis=1)
        return weighted_preds, weighted_probs

    def predict_hierarchical(self, mlp_results, logreg_results, cnn_results):
        mlp_preds, mlp_probs = mlp_results
        logreg_preds, logreg_probs = logreg_results
        cnn_preds, cnn_probs = cnn_results
        
        # Get confidence scores
        mlp_conf = np.max(mlp_probs, axis=1)
        
        # Initialize arrays for final predictions
        n_samples = len(mlp_preds)
        final_preds = np.zeros(n_samples, dtype=int)
        final_probs = np.zeros_like(mlp_probs)
        
        # Hierarchical decision making
        for i in range(n_samples):
            if mlp_conf[i] > 0.8:  # If MLP is very confident
                final_preds[i] = mlp_preds[i]
                final_probs[i] = mlp_probs[i]
            elif logreg_preds[i] == cnn_preds[i]:  # If LogReg and CNN agree
                final_preds[i] = logreg_preds[i]
                final_probs[i] = (logreg_probs[i] + cnn_probs[i]) / 2
            else:  # Otherwise use weighted average
                final_probs[i] = (
    ...
    def predict_all(self, images, clip_features):
        ...
        results['weighted_voting'] = self.predict_weighted_voting(
            results['mlp'], results['logreg'], results['cnn']
        )
        results['hierarchical'] = self.predict_hierarchical(
            results['mlp'], results['logreg'], results['cnn']
        )
    ...
    if uploaded_file is not None:
        ...
        with tabs[0]:
            ...
            - **Hierarchical**: Uses a decision tree approach:
                ...
                2. If LogReg and CNN agree, use their average
        ...
        with tabs[2]:
            ...
            for model_name, (preds, probs) in all_models.items():
